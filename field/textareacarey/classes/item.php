...
/**
 * This file contains the surveyprofield_textareacarey
 *
...
/**
 * Class to manage each aspect of the textareacarey item
 *
 * @package   surveyprofield_textareacarey
 * @copyright 2013 onwards kordan <stringapiccola@gmail.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class item extends itembase {
    ...
    public function item_add_mandatory_plugin_fields(&$record) {
        ...
        $record->variable = 'textareacarey_001';
        $record->indent = 0;
        $record->useeditor = 0;
        $record->arearows = 10;
        $record->areacols = 60;
    ...
    public function item_custom_fields_to_db($record) {
        ...
        if (empty($record->arearows)) {
            $record->arearows = SURVEYPROFIELD_textareacarey_DEFAULTROWS;
        }
        if (empty($record->areacols)) {
            $record->areacols = SURVEYPROFIELD_textareacarey_DEFAULTCOLS;
        }
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema" elementFormDefault="qualified">
    <xs:element name="surveyprofield_textareacarey">
        <xs:complexType>
            <xs:sequence>
...
EOS;
    ...
    public function userform_mform_element($mform, $searchform, $readonly) {
        ...
        $idprefix = 'id_surveypro_field_textareacarey_'.$this->sortindex;
        ...
        if (empty($this->useeditor) || ($searchform)) {
            ...
            $attributes['class'] = 'indent-'.$this->indent.' textareacarey_textareacarey';
            $attributes['wrap'] = 'virtual';
            if (!$searchform) {
                $mform->addElement('mod_surveypro_textareacarey_plain', $fieldname, $elementlabel, $attributes);
                $mform->setType($fieldname, PARAM_TEXT);
            } else {
                $elementgroup = [];
                $elementgroup[] = $mform->createElement('mod_surveypro_textareacarey_plain', $fieldname, $elementlabel, $attributes);

                $itemname = $this->itemname.'_ignoreme';
                $starstr = get_string('star', 'mod_surveypro');
                $attributes['id'] = $idprefix.'_ignoreme';
                $attributes['class'] = 'textareacarey_check';
            ...
            $attributes['class'] = 'indent-'.$this->indent.' textareacarey_editor';
            $fieldname = $this->itemname.'_editor';
            $editoroptions = ['trusttext' => true, 'subdirs' => true, 'maxfiles' => EDITOR_UNLIMITED_FILES];
            $mform->addElement('mod_surveypro_textareacarey_editor', $fieldname, $elementlabel, $attributes, $editoroptions);
    ...
    public function userform_mform_validation($data, &$errors, $searchform) {
        ...
        if ( $this->maxlength && (\core_text::strlen($itemcontent) > $this->maxlength) ) {
            $errors[$errorkey] = get_string('uerr_texttoolong', 'surveyprofield_textareacarey');
        }
        if (\core_text::strlen($itemcontent) < $this->minlength) {
            $errors[$errorkey] = get_string('uerr_texttooshort', 'surveyprofield_textareacarey');
        }
    }
    ...
    public function userform_get_filling_instructions() {
        ...
        if ($this->minlength > 0) {
            if (isset($this->maxlength) && ($this->maxlength > 0)) {
                ...
                $arrayinstruction[] = get_string('hasminmaxlength', 'surveyprofield_textareacarey', $a);
            } else {
                $a = $this->minlength;
                $arrayinstruction[] = get_string('hasminlength', 'surveyprofield_textareacarey', $a);
            }
        } else {
            if (isset($this->maxlength) && ($this->maxlength > 0)) {
                $a = $this->maxlength;
                $arrayinstruction[] = get_string('hasmaxlength', 'surveyprofield_textareacarey', $a);
            }
        }
        if ($this->trimonsave) {
    ...
    public function userform_get_user_answer($answer, &$olduseranswer, $searchform) {
        if (!empty($this->useeditor) && (!$searchform)) {
            ...
            $olduseranswer = file_postupdate_standard_editor($olduseranswer, $this->itemname, $editoroptions, $context,
                    'mod_surveypro', SURVEYPROFIELD_textareacarey_FILEAREA, $olduseranswer->id);
        ...
        if (isset($fromdb->content)) {
            if (!empty($this->useeditor)) {
                ...
                $fromdb = file_prepare_standard_editor($fromdb, 'content', $editoroptions, $context,
                                                       'mod_surveypro', SURVEYPROFIELD_textareacarey_FILEAREA, $fromdb->id);
        ...
        if (core_text::strlen($content)) {
            if ($this->useeditor) {
                $content = file_rewrite_pluginfile_urls(
                           ...
                           'mod_surveypro', SURVEYPROFIELD_textareacarey_FILEAREA, $answer->id);
