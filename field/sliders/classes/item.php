...
/**
 * This file contains the surveyprofield_sliders
 *
...
/**
 * Class to manage each aspect of the sliders item
 *
 * @package   surveyprofield_sliders
 * @copyright 2013 onwards kordan <stringapiccola@gmail.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class item extends itembase {
     ...
     *
     * Take in mind that "required" (for sliders item) means only: "The 'No answer' sliders is not displayed".
     * Each sliders element is intrinsically required.
     * There is noy for a student to jump a standard sliders element.
     *
     ...
     * In that way the element will be equipped with an additional exclusive "No answer" sliders.
     * This last sliders privides to the student the possibility to say "I don't tell you what I take for breakfast!"
     *
     * Note that a sliders can have $minimumrequired irrespectively of being required or no.
     * Alias: a sliders element can be not $required and have, at the same time, $minimumrequired > 0.
     * This is perfectly valid.
     *
     * Example: "What do you take for breakfast?" milk, bread, jam.
     * With: $required = 0 and $minimumrequired = 2
     *
     * This means that the student is allowed to select the exclusive "No answer" sliders and run away BUT
     * IF he/she decides to provide an answer THEN he/she has to select at least 2 sliderses.
     */
    protected $required;
    ...
    /**
     * @var int Mminimum number of sliderses the user is forced to choose in his/her answer
     */
    protected $minimumrequired;

    /**
     * @var int Maximum number of sliderses the user is forced to choose in his/her answer
     */
    ...
    public function item_add_mandatory_plugin_fields(&$record) {
        $record->content = 'sliders';
        $record->contentformat = 1;
        $record->position = 0;
        $record->required = 0;
        $record->hideinstructions = 0;
        $record->variable = 'sliders_001';
        $record->indent = 0;
    ...
    public function item_custom_fields_to_db($record) {
        ...
        // 3. Set values corresponding to sliderses.
        // Take care: 'required', 'hideinstructions' were already considered in get_common_settings.
        $sliderses = ['noanswerdefault'];
        foreach ($sliderses as $sliders) {
            $record->{$sliders} = (isset($record->{$sliders})) ? 1 : 0;
        }
    ...
    public function item_list_constraints() {
        ...
        $optionstr = get_string('option', 'surveyprofield_sliders');
        foreach ($values as $value) {
            $constraints[] = $optionstr.$labelsep.$value;
        }
        if (!empty($this->labelother)) {
            $labelotherstr = get_string('labelother', 'surveyprofield_sliders');
            $allowedstr = get_string('allowed', 'surveyprofield_sliders');
    ...
    public function get_downloadformats() {
        ...
        $options[SURVEYPRO_ITEMSRETURNSVALUES] = get_string('returnvalues', 'surveyprofield_sliders');
        $options[SURVEYPRO_ITEMRETURNSLABELS] = get_string('returnlabels', 'surveyprofield_sliders');
        $options[SURVEYPRO_ITEMRETURNSPOSITION] = get_string('returnposition', 'surveyprofield_sliders');
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema" elementFormDefault="qualified">
    <xs:element name="surveyprofield_sliders">
        <xs:complexType>
            <xs:sequence>
...
EOS;
    ...
    public function parent_validate_child_constraints($childparentvalue) {
        ...
        if ($key !== false) {
            ...
            $condition = $condition && ($expectedcount == $key); // Only $expectedcount sliderses are allowed.
    ...
    public function userform_mform_element($mform, $searchform, $readonly) {
        ...
        $idprefix = 'id_surveypro_field_sliders_'.$this->sortindex;
        ...
        $attributes = [];
        $attributes['class'] = 'indent-'.$this->indent.' sliders_check';
        $attributes['group'] = 1;
        ...
        foreach ($labels as $label) {
            ...
            $elementgroup[] = $mform->createElement('mod_surveypro_advsliders', $itemname, '', $label, $attributes, $options);

            if ($this->adjustment == SURVEYPRO_HORIZONTAL) {
                $attributes['class'] = 'sliders_check';
            }
        ...
        if (!empty($this->labelother)) {
            ...
            $elementgroup[] = $mform->createElement('mod_surveypro_advsliders', $itemname, '', $otherlabel, $attributes, $options);
        ...
        if (!$this->required) {
            $elementtype = 'mod_surveypro_advsliders';
        ...
        if ($searchform) {
            ...
            $mform->addElement('mod_surveypro_sliders', $itemname, '', get_string('star', 'mod_surveypro'), $attributes);
            $mform->setDefault($itemname, '1');
     ...
     * sliders type items with the "Other" option in addition to a single option
    ...
    public function userform_mform_validation($data, &$errors, $searchform) {
        ...
        if (!empty($this->labelother)) {
            if (($data[$this->itemname.'_other']) && empty($data[$this->itemname.'_text']) ) {
                $errors[$errorkey] = get_string('uerr_missingothertext', 'surveyprofield_sliders');
                return;
            }
        }
        ...
        foreach ($labels as $k => $unused) {
            ...
            if ($data[$itemname]) { // They are advanced sliders so I am sure the answer always exist.
                $answercount++;
            }
        }
        ...
        if ($answercount < $this->minimumrequired) {
            if ($this->minimumrequired == 1) {
                $errors[$errorkey] = get_string('uerr_lowerthanminimum_one', 'surveyprofield_sliders');
            } else {
                $errors[$errorkey] = get_string('uerr_lowerthanminimum_more', 'surveyprofield_sliders', $this->minimumrequired);
            }
        }
        if (!empty($this->maximumrequired) && ($answercount > $this->maximumrequired)) {
            if ($this->maximumrequired == 1) {
                $errors[$errorkey] = get_string('uerr_greaterthanmaximum_one', 'surveyprofield_sliders');
            } else {
                $errors[$errorkey] = get_string('uerr_greaterthanmaximum_more', 'surveyprofield_sliders', $this->maximumrequired);
            }
        }
    }
    ...
    public function userform_get_parent_disabilitation_info($childparentvalue) {
        ...
        // If this item foresees the "No answer" sliders, provide a directive for it too.
        if (!$this->required) {
            $mformelementinfo = new \stdClass();
    ...
    public function userform_is_child_allowed_dynamic($childparentvalue, $data) {
        // I need to verify (sliders per sliders) if they hold the same value the user entered.
    ...
    public function userform_get_filling_instructions() {
        ...
        if (!empty($this->minimumrequired)) {
            if ($this->minimumrequired == 1) {
                $arrayinstruction[] = get_string('restrictions_minimumrequired_one', 'surveyprofield_sliders');
            } else {
                $a = $this->minimumrequired;
                $arrayinstruction[] = get_string('restrictions_minimumrequired_more', 'surveyprofield_sliders', $a);
            }
        }
        if (!empty($this->maximumrequired)) {
            if ($this->maximumrequired == 1) {
                $arrayinstruction[] = get_string('restrictions_maximumrequired_one', 'surveyprofield_sliders');
            } else {
                $a = $this->maximumrequired;
                $arrayinstruction[] = get_string('restrictions_maximumrequired_more', 'surveyprofield_sliders', $a);
            }
        }
    ...
    public function userform_get_user_answer($answer, &$olduseranswer, $searchform) {
        ...
        if (isset($answer['ignoreme']) && ($answer['ignoreme'] == 1)) { // It is an advsliders.
    ...
    public function userform_get_prefill($fromdb) {
        ...
        if (isset($fromdb->content)) {
            ...
            foreach ($answers as $k => $slidersvalue) {
                $itemname = $this->itemname.'_'.$k;
                $prefill[$itemname] = $slidersvalue;
            }
            if (!empty($this->labelother)) {
        ...
        // If the "No answer" sliders is part of the element GUI...
